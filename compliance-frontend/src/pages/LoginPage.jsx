    ...
    <div className="flex min-h-screen font-display">
      ...
      <div className="w-[44%] min-w-[340px] bg-gradient-to-br from-[#0F7B5A] via-primary to-[#0B8A62] p-10 flex flex-col relative overflow-hidden max-md:w-full max-md:min-w-0 max-md:p-7">
        ...
        <div className="flex-1 flex flex-col justify-center py-8 z-10 max-md:py-6">
          <h1 className="text-[2rem] font-extrabold text-white leading-[1.2] tracking-tight mb-3.5 max-md:text-2xl">Your pre-certification<br />compliance lab</h1>
          <p className="text-white/80 text-[0.97rem] leading-relaxed mb-7">Test products against BIS and FSSAI regulations before costly lab submissions.</p>
          <ul className="flex flex-col gap-2.5 list-none max-md:hidden">
            {['5-module lab-style compliance testing', 'Explainable risk scores with breakdown', 'AI-powered executive summary', 'PDF compliance certificate'].map((text) => (
      ...
      <div className="flex-1 flex items-center justify-center bg-white p-8 max-md:p-5">
        <div className="w-full max-w-[420px]">
          <div className="mb-7">
            ...
            <p className="text-slate-500 mt-1 text-[0.95rem]">Sign in to continue to your dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                ...
                <Link to="/forgot-password" className="text-xs font-semibold text-primary hover:underline transition-colors">Forgot password?</Link>
              </div>
            ...
            <button type="submit" className="mt-1.5 w-full py-3 text-[0.97rem] font-bold text-white bg-gradient-to-r from-primary to-[#0B8A62] border-none rounded-lg cursor-pointer flex items-center justify-center gap-2 transition-all shadow-lg shadow-primary/30 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-primary/40 disabled:opacity-65 disabled:cursor-not-allowed disabled:translate-y-0" disabled={loading}>
              {loading ? <><span className="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Signing In...</> : 'Sign In'}
            </button>
          </form>

          <p className="text-center mt-5 text-sm text-slate-500">
            Don't have an account? <Link to="/signup" className="text-primary font-semibold no-underline hover:underline">Create one →</Link>
          </p>
